...
function makeSeoBlock(tool, intent, url, baseUrl) {
  ...
  // Simple deterministic rating seed based on tool slug
  const ratingHash = (tool.name.length * 7 + tool.slug.charCodeAt(0) * 13) % 20;
  const ratingValue = (4.7 + (ratingHash / 100)).toFixed(2); // ratings between 4.70 and 4.89
  const reviewCount = 80 + (ratingHash * 8); // review counts between 80 and 240
  ...
  // Generate collapsible internal linking drawer elements
  const baseLink = `<a href="${tool.file}" style="padding: 8px 12px; border: 1px solid var(--primary); background: rgba(94, 234, 212, 0.05); border-radius: 20px; font-size: 0.85em; color: var(--primary); text-decoration: none; transition: all 0.2s; white-space: nowrap; text-align: center; display: inline-block; font-weight: 600;" onmouseover="this.style.background='var(--primary)';this.style.color='#fff'" onmouseout="this.style.background='rgba(94, 234, 212, 0.05)';this.style.color='var(--primary)'">${escapeHtml(tool.name)} (Base Tool)</a>`;
  
  const otherLinks = intents.map(otherIntent => {
    if (otherIntent.suffix === intent.suffix) return '';
    const filename = `${tool.slug}-${otherIntent.suffix}.html`;
    return `<a href="${filename}" style="padding: 8px 12px; border: 1px solid var(--border); border-radius: 20px; font-size: 0.85em; color: var(--muted); text-decoration: none; transition: all 0.2s; white-space: nowrap; text-align: center; display: inline-block;" onmouseover="this.style.borderColor='var(--primary)';this.style.color='var(--primary)'" onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--muted)'">${escapeHtml(tool.name)} ${escapeHtml(otherIntent.label)}</a>`;
  }).filter(Boolean).join('\n');
